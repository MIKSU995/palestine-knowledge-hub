...
@section('page_title', 'Community Moderation')
@section('page_subtitle', 'Approve comments, manage reported content, and uphold community standards')
...
<div class="bg-white rounded-3xl border border-slate-200 shadow-sm overflow-hidden mb-8">
    <div class="p-6 border-b border-slate-100 flex items-center justify-between">
        <div>
            ...
            <p class="text-xs text-slate-400 mt-0.5">User-flagged inappropriate or inaccurate content</p>
        </div>
        <span class="text-xs font-bold text-slate-400">{{ $reports->total() }} reports</span>
    </div>
    ...
    <div class="overflow-x-auto">
        <table class="w-full text-left text-sm">
            <thead class="bg-slate-50 text-slate-500 font-semibold border-b border-slate-100">
                <tr>
                    <th class="p-4 pl-6">Reported By</th>
...
<div class="bg-white rounded-3xl border border-slate-200 shadow-sm overflow-hidden">
    <div class="p-6 border-b border-slate-100 flex items-center justify-between">
        <div>
            ...
            <p class="text-xs text-slate-400 mt-0.5">Approve, reject, or delete community discussion comments</p>
        </div>
        <span class="text-xs font-bold text-slate-400">{{ $allComments->total() }} comments total</span>
    </div>
    ...
    <div class="overflow-x-auto">
        <table class="w-full text-left text-sm">
            ...
            <tbody class="divide-y divide-slate-100">
                ...
                <tr class="hover:bg-slate-50 transition">
                    <td class="p-4 pl-6">
                        <p class="font-semibold text-slate-900">{{ $comment->user->name ?? 'Guest' }}</p>
                    ...
                    <td class="p-4 pr-6 text-right">
                        <div class="flex items-center justify-end gap-2">
                            ...
                            <form action="{{ route('admin.moderation.comment.delete', $comment->id) }}" method="POST" onsubmit="return confirm('Delete comment permanently?')">
                                @csrf
                                @method('DELETE')
