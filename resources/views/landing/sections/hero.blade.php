        ...
        <div class="grid lg:grid-cols-2 gap-12 lg:gap-16 items-center">
            ...
            <div class="space-y-6">
                ...
                <div class="flex flex-wrap gap-4 pt-2">
                    <a href="{{ route('articles') }}" class="px-7 py-3.5 rounded-2xl bg-emerald-600 hover:bg-emerald-500 text-white font-bold text-base transition shadow-lg shadow-emerald-900/30">
                        Explore Articles
                    </a>

                    <a href="{{ route('maps') }}" class="px-7 py-3.5 rounded-2xl bg-slate-800 hover:bg-slate-700 text-slate-200 border border-slate-700 font-bold text-base transition flex items-center gap-2">
                ...
                <div class="grid grid-cols-4 gap-4 pt-6 border-t border-slate-800 text-center">
                    <div>
                        <div class="text-2xl font-bold text-white">{{ $stats['total_articles'] ?? '12+' }}</div>
                        <div class="text-xs text-slate-400">Articles</div>
                    </div>
                    <div>
                        <div class="text-2xl font-bold text-emerald-400">{{ $stats['total_timeline'] ?? '10+' }}</div>
                        <div class="text-xs text-slate-400">Timeline Events</div>
                    </div>
                    <div>
                        <div class="text-2xl font-bold text-amber-400">{{ $stats['total_resources'] ?? '8+' }}</div>
                        <div class="text-xs text-slate-400">Resources</div>
                    </div>
                    <div>
                        <div class="text-2xl font-bold text-purple-400">{{ $stats['total_quizzes'] ?? '5+' }}</div>
