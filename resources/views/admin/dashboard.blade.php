...
@section('page_subtitle', 'Ringkasan statistik dan aktivitas Palestine Knowledge Hub')
...
<div class="grid grid-cols-2 xl:grid-cols-4 gap-5 mb-8">
    ...
    <div class="bg-white rounded-3xl border border-slate-200 p-6 shadow-sm hover:shadow-md transition">
        <div class="flex items-center justify-between mb-4">
            ...
            <span class="text-xs font-semibold text-blue-600 bg-blue-50 px-2 py-1 rounded-lg">Pengguna</span>
        </div>
        <p class="text-4xl font-extrabold text-slate-900">{{ $totalUsers }}</p>
        <p class="text-sm text-slate-400 mt-1.5">Pengguna terdaftar</p>
    </div>
    ...
    <div class="bg-white rounded-3xl border border-slate-200 p-6 shadow-sm hover:shadow-md transition">
        <div class="flex items-center justify-between mb-4">
            ...
            <span class="text-xs font-semibold text-purple-600 bg-purple-50 px-2 py-1 rounded-lg">Kuis</span>
        </div>
        <p class="text-4xl font-extrabold text-slate-900">{{ $totalQuizzes }}</p>
        <p class="text-sm text-slate-400 mt-1.5">Kuis interaktif</p>
    </div>
    ...
    <div class="bg-white rounded-3xl border border-slate-200 p-6 shadow-sm hover:shadow-md transition {{ $pendingReports > 0 ? 'border-red-200 bg-red-50' : '' }}">
        <div class="flex items-center justify-between mb-4">
            ...
            <span class="text-xs font-semibold {{ $pendingReports > 0 ? 'text-red-600 bg-red-100' : 'text-slate-600 bg-slate-100' }} px-2 py-1 rounded-lg">Laporan</span>
        </div>
        <p class="text-4xl font-extrabold {{ $pendingReports > 0 ? 'text-red-700' : 'text-slate-900' }}">{{ $pendingReports }}</p>
        <p class="text-sm {{ $pendingReports > 0 ? 'text-red-400' : 'text-slate-400' }} mt-1.5">Menunggu peninjauan</p>
    </div>
...
<div class="grid lg:grid-cols-2 gap-6">
    ...
    <div class="bg-white rounded-3xl border border-slate-200 shadow-sm overflow-hidden">
        <div class="flex items-center justify-between px-6 py-5 border-b border-slate-100">
            ...
            <a href="{{ route('admin.moderation.index') }}" class="text-xs font-bold text-red-500 hover:text-red-600 hover:underline">Lihat Semua</a>
        </div>
        <div class="divide-y divide-slate-50">
            ...
            <div class="px-6 py-4 flex items-start gap-3 hover:bg-slate-50 transition">
                ...
                <div class="flex-1 min-w-0">
                    ...
                    <p class="text-xs text-slate-400 mt-0.5">
                        Oleh {{ $report->user->name ?? 'Tamu' }} • {{ $report->created_at->diffForHumans() }}
                    </p>
                </div>
            ...
            @empty
            <div class="px-6 py-10 text-center text-slate-400 text-sm">
                Belum ada laporan. Semuanya aman! ✅
            </div>
            @endforelse
        </div>
    </div>
